...
*** Data
# More measures: number of times unit has been relocated by a party in 1997,2006,2013,2018 (Trelles); more on unit volatility (emm), ENP/Molinar?
Measures of federal deputy election at two geographic levels are distributed: the units of observation in this set of files are municipalities (files with ~municipio~ in name) or secciones electorales (files with ~seccion~ in name). Quantities of interest are available for 2009, 2012, 2015, and 2018. Variable descriptions [[codebookElec][here]].
