# copié el código q usé para Merino y lo adapté para añadir distritos locales

# ponga aquí la ruta donde guardará los archivos distribuidos
setwd("/home/eric/Dropbox/data/elecs/MXelsCalendGovt/atlasDis/claseMerino/")

# limpia memoria
rm(list = ls())
# get DF mun names and votes
d <- read.csv(file = "data/aymu1977-present.csv", stringsAsFactors = FALSE)
#load("data/aymu1977-present.RData")
munvot <- d
rm(list=setdiff(ls(), "munvot")) # clean
#
load(file="data/elDatForMaps.RData")
...
sec2dis <- read.csv("data/tablaEquivalenciasSeccionales1994-2010.2013.csv", stringsAsFactors = FALSE)
sec2dis <- sec2dis[sec2dis$edon == 9,]
# send to seccion map
...
rm(tmp)
#                                                     ###################################################################################
# di.map <- unionSpatialPolygons(se.map, se.map$disn) # proper way to get federal district objects... but seccion shapefiles have problems
#                                                     ###################################################################################
#
...
plot(ed.map$df, axes = TRUE, main = "Distrito Federal (mapa 2006)")
...
text( x = -99.2, y = 19.05, labels = "MORELOS", col = "darkgray", cex = .9 )
text( x = -99.35, y = 19.5, labels = "MEXICO", col = "darkgray", cex = .9, srt = 45 )
text( x = -98.95, y = 19.5, labels = "MEXICO", col = "darkgray", cex = .9, srt = -45 )
## dev.off()
