...
mu.map$mun <- c("ABASOLO","AGUALEGUAS","LOS ALDAMAS","ALLENDE","ANAHUAC","ARAMBERRI","CADEREYTA JIMENEZ","CARMEN","CERRALVO","CIENEGA DE FLORES","CHINA","DR. ARROYO","DR. COSS","DR. GONZALEZ","GALEANA","GARCIA","SAN PEDRO GARZA GARCIA","GRAL. BRAVO","GRAL. TERAN","GRAL. TREVIÑO","GRAL. ZARAGOZA","GRAL. ZUAZUA","GUADALUPE","LOS HERRERAS","HIDALGO","HIGUERAS","HUALAHUISES","ITURBIDE","LAMPAZOS DE NARANJO","LINARES","MARIN","MELCHOR OCAMPO","MIER Y NORIEGA","MINA","MONTEMORELOS","MONTERREY","PARAS","LOS RAMONES","RAYONES","SABINAS HIDALGO","SALINAS VICTORIA","SAN NICOLAS DE LOS GARZA","SANTA CATARINA","SANTIAGO","VALLECILLO","VILLALDAMA","BUSTAMANTE","JUAREZ","PESQUERIA","APODACA","GRAL. ESCOBEDO")
...
sec2dis <- sec2dis[sec2dis$edon == 19,]
# send to seccion map
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon== 19, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon == 19,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/nl2015.csv", stringsAsFactors = FALSE) # 2015 districts
...
#
# Estado
tmp <- as(ed.map$nl, "SpatialLinesDataFrame") # coerce polygons to lines 
...
plot(ed.map$nl, axes = TRUE, main = "Nuevo León (mapa 2006)")
plot(ed.map$tam, add = TRUE, lty = 3)
plot(ed.map$coa, add = TRUE, lty = 3)
...
# thick state border
plot(ed.map$nl , add = TRUE, lwd = 3)
plot(ed.map$nl , add = TRUE, border = "red", lty = 3, lwd = 2)
...
text( x = -97.25, y = 24, labels = "G O L F O   D E   M E X I C O", cex = .9, col = "deepskyblue", srt = 85 )
text( x = -98, y = 27, labels = "E.E.U.U.", col = "darkgray", cex = .9 )
text( x = -98.5, y = 24.25, labels = "TAMAULIPAS", col = "darkgray", cex = .9 )
text( x = -101, y = 23.5, labels = "S.L.P.", col = "darkgray", cex = .9 )
text( x = -102, y = 24.25, labels = "ZACATECAS", col = "darkgray", cex = .9, srt = 0 )
text( x = -102, y = 26.5, labels = "COAHUILA", col = "darkgray", cex = .9, srt = 0 )
## dev.off()
...
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon == 19 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Nuevo León", tmp))
plot(ed.map$nl , add = TRUE)
...
plot(ed.map$coa, add = TRUE, lty = 1)
...
loc <- c("bottomleft","topright","bottomleft","topright","topright",       #  5
         "topright","bottomright","topright","bottomright","bottomleft",   # 10
         "topright","topright") 
## pdf(file = paste(md2, edo, dn, "-3.pdf", sep = ""))
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon == 19 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Nuevo León", dn, "-", tmp))
plot(ed.map$nl , add = TRUE)
