...
# read disloc2005
dl2011.map <- readOGR(dsn = tmp, layer = 'disloc2011')
colnames(dl2011.map@data) <- c("edon","edo","disloc")
# projects to a different datum with long and lat
dl2011.map <- spTransform(dl2011.map, osm()) # project to osm native Mercator

# add father/son info and dsi of mapLoc
dsi <- "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/ife.ine/mapasComparados/loc/simIndex/dist_coa.csv"
...
sec2dis <- read.csv("/home/eric/Dropbox/data/elecs/MXelsCalendGovt/redistrict/ife.ine/mapasComparados/loc/coaLoc.csv", stringsAsFactors = FALSE)
# send to seccion map
...
tmp <- merge(x = tmp, y = se.map[,c("SECCION","disfed2006","disfed2018","disloc2011","disloc2017")], by = "SECCION", all.x = TRUE, all.y = FALSE)
...
# clean mun labels, they have numbers...
...
plot(p84(ed.map$zac), add = TRUE, lty = 3)
plot(p84(ed.map$nl), add = TRUE, lty = 3)
plot(p84(ed.map$tam), add = TRUE, lty = 3)
plot(p84(ed.map$san), add = TRUE, lty = 3)
# 
plot(p84(dl.map), add = TRUE, border = "gray")
plot(p84(dl.map[dl.map$disloc==dn,]), add = TRUE, border = "gray", col = "hotpink")
# thick state border
plot(p84(ed.map$coa), add = TRUE, lwd = 3)
plot(p84(ed.map$coa), add = TRUE, border = "red", lty = 3, lwd = 2)
...
text( x = -100, y = 26, labels = "NUEVO LEON", col = "darkgray", cex = .9 )
text( x = -99.5, y = 29, labels = "EE.UU.", col = "darkgray", cex = .9)
text( x = -99.28, y = 26.55, labels = "TAMAULIPAS", col = "darkgray", cex = .9, srt = -58)
text( x = -101.75, y = 24.6, labels = "ZACATECAS", col = "darkgray", cex = .9)
text( x = -104.5, y = 25.5, labels = "DURANGO", col = "darkgray", cex = .9)
text( x = -104.6, y = 28, labels = "CHIHUAHUA", col = "darkgray", cex = .9)
#dev.off()
...
plot(dl.map[dl.map$disloc==dn,], axes = TRUE, main = paste("Coahuila ", dn, " - ", tmp, " (DSI = ", tmp2, ")", sep = ""))
plot(bg, add = TRUE)
#plot(dl.map[dl.map$disloc==dn,], lwd = 5, add = TRUE) # drop
plot(ed.map$coa, add = TRUE)
...
#
#
plot(ed.map$cua, add = TRUE, lty = 1)
plot(ed.map$dgo, add = TRUE, lty = 1)
plot(ed.map$zac, add = TRUE, lty = 1)
plot(ed.map$nl, add = TRUE, lty = 1)
plot(ed.map$tam, add = TRUE, lty = 1)
#
sel <- which(dl2011.map$disloc==dl.map$father[dl.map$disloc==dn])
plot(dl2011.map[sel,], add = TRUE, lwd = 6, border = "red")
#
plot(dl.map[dl.map$disloc==dn,], add = TRUE, lwd = 4)
plot(mu.map, add = TRUE, border = "green", lwd = 1)
plot(mu.map, add = TRUE, lwd = 1, lty = 3)
plot(ed.map$coa, add = TRUE, lwd = 3)
plot(ed.map$coa, add = TRUE, border = "red", lty = 3, lwd = 2)
