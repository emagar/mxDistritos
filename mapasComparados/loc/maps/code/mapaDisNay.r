...
# projects to a different datum with long and lat
cab <- spTransform(cab, osm()) # project to osm native Mercator
...
mu.map$mun <- c("RUIZ","ROSAMORADA","TUXPAN","LA YESCA","TECUALA","ACAPONETA","HUAJICORI","EL NAYAR","SAN BLAS","XALISCO","TEPIC","COMPOSTELA","B. DE BANDERAS","S. PEDRO LAG.","JALA","IXTLAN DEL RIO","AMATLAN DE C.","SGO. IXCUINTLA","S. MARIA DEL ORO","AHUACATLAN")
...
dl.map <- readOGR(dsn = tmp, layer = 'disloc2017')
colnames(dl.map@data) <- c("edon","edo","disloc")
# projects to a different datum with long and lat
dl.map <- spTransform(dl.map, osm()) # project to osm native Mercator
# read disloc2005
dl2005.map <- readOGR(dsn = tmp, layer = 'disloc2005')
colnames(dl2005.map@data) <- c("edon","edo","disloc")
# projects to a different datum with long and lat
dl2005.map <- spTransform(dl2005.map, osm()) # project to osm native Mercator
# read demarcaciones 2008
dem.map <- readOGR(dsn = tmp, layer = 'demarc2008') # ojo: éste no abre
colnames(dem.map@data) <- c("edon","edo","disloc")
# projects to a different datum with long and lat
dem.map <- spTransform(dem.map, osm()) # project to osm native Mercator

# add father/son info and dsi of mapLoc
dsi <- "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/ife.ine/mapasComparados/loc/simIndex/dist_nay.csv"
dsi <- read.csv(file = dsi, stringsAsFactors = FALSE)
#
#dl.map$ord <- 1:nrow(dl.map@data)
dl.map@data <- merge(x = dl.map@data, y = dsi, by.x = "disloc", by.y = "disloc2017", all.x = TRUE, all.y = FALSE)
rm(dsi)
...
sec2dis <- read.csv("/home/eric/Dropbox/data/elecs/MXelsCalendGovt/redistrict/ife.ine/mapasComparados/loc/nayLoc.csv", stringsAsFactors = FALSE)
# send to seccion map
...
# projects to a different datum with long and lat
di.map <- spTransform(di.map, osm()) # project to osm native Mercator
...
tmp <- merge(x = tmp, y = se.map[,c("SECCION","disfed2006","disfed2018","disloc2005","disloc2017")], by = "SECCION", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon== 18, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon==18,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
#tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/nay2014.csv", stringsAsFactors = FALSE) # 2015 districts
#tmp1 <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/nay2014demarc.csv", stringsAsFactors = FALSE) # 2015 districts
#tmp$disloc <- tmp$disn; tmp$disn <- NULL
## table(is.na(tmp$disloc2017))             # verify missing
## tmp$SECCION[is.na(tmp$disloc2017)==TRUE] # verify missing
#se.map$disloc <- se.map$disloc2017#; se.map$cabloc <- tmp$cabloc
#
# add seccion volat 2012-2015 = max change
# add lisnom2015
...
plot(p84(ed.map$dgo), col = "white", add = TRUE, lty = 3)
# 
plot(p84(dl.map), add = TRUE, border = "gray")
plot(p84(dl.map[dl.map$disloc==dn,]), add = TRUE, border = "gray", col = "gray")
# thick state border
plot(p84(ed.map$nay), add = TRUE, lwd = 3)
plot(p84(ed.map$nay), add = TRUE, border = "red", lty = 3, lwd = 2)
...
text( x = -106, y = 21.75, labels = "O C E A N O   P A C I F I C O", cex = .9, col = "deepskyblue", srt = -60 )
text( x = -104.25, y = 20.5, labels = "JALISCO", col = "darkgray", cex = .9 )
text( x = -103.9, y = 22, labels = "JALISCO", col = "darkgray", cex = .9, srt = -45 )
...
#
plot(ed.map$sin, add = TRUE, lty = 1)
plot(ed.map$jal, add = TRUE, lty = 1)
#plot(ed.map$zac, add = TRUE, lty = 1)
plot(ed.map$dgo, add = TRUE, lty = 1)
#
sel <- which(dl2005.map$disloc==dl.map$father[dl.map$disloc==dn])
plot(dl2005.map[sel,], add = TRUE, lwd = 6, border = "red")
#
plot(dl.map[dl.map$disloc==dn,], add = TRUE, lwd = 4)
plot(mu.map, add = TRUE, border = "green", lwd = 1)
plot(mu.map, add = TRUE, lwd = 1, lty = 3)
plot(ed.map$nay, add = TRUE, lwd = 3)
plot(ed.map$nay, add = TRUE, border = "red", lty = 3, lwd = 2)
...
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon==18 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Nayarit", dn, "-", tmp))
plot(ed.map$nay, add = TRUE)
