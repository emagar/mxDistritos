...
mu.map$mun <- c("HUEJUQUILLA",           
"MEZQUITIC",                    
"VILLA GUERRERO",                
"BOLAÑOS",                   
"TOTATICHE",                     
"COLOTLAN",                     
"STA MARIA DE LOS ANGELES",    
"HUEJUCAR",                     
"CHIMALTITAN",                   
"SN MARTIN BOLAÑOS",     
"OJUELOS DE JALISCO",            
...
sec2dis <- sec2dis[sec2dis$edon == 14,]
# send to seccion map
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon== 14, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon == 14,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/jal2012.csv", stringsAsFactors = FALSE) # 2015 districts
tmp$disloc <- tmp$dl; tmp$dl <- NULL
...
#
# Estado
tmp <- as(ed.map$jal, "SpatialLinesDataFrame") # coerce polygons to lines 
...
plot(ed.map$jal, axes = TRUE, main = "Jalisco (mapa 2006)")
plot(ed.map$nay, add = TRUE, lty = 3)
plot(ed.map$sin, add = TRUE, lty = 3)
plot(ed.map$dgo, add = TRUE, lty = 3)
plot(ed.map$ags, add = TRUE, lty = 3)
plot(ed.map$gua, add = TRUE, lty = 3)
plot(ed.map$col, add = TRUE, lty = 3)
...
# thick state border
plot(ed.map$jal, add = TRUE, lwd = 3)
plot(ed.map$jal, add = TRUE, border = "red", lty = 3, lwd = 2)
...
text( x = -105.1, y = 19.15, labels = "O C E A N O   P A C I F I C O", cex = .9, col = "deepskyblue", srt = -28 )
text( x = -103.1, y = 20.235, labels = "L. Chapala", cex = .7, col = "deepskyblue", srt = 0 )
text( x = -104.7, y = 21.7, labels = "NAYARIT", col = "darkgray", cex = .9, srt = 0 )
text( x = -104.55, y = 22.8, labels = "DURANGO", col = "darkgray", cex = .9, srt = 0 )
text( x = -102.5, y = 22.7, labels = "ZACATECAS", col = "darkgray", cex = .9, srt = 0 )
text( x = -102.35, y = 22, labels = "AGS.", col = "darkgray", cex = .9, srt = 0 )
text( x = -101.6, y = 20.8, labels = "GUANAJUATO", col = "darkgray", cex = .9, srt = 60 )
text( x = -102, y = 19.2, labels = "MICHOACAN", col = "darkgray", cex = .9, srt = 0 )
text( x = -103.9, y = 19.1, labels = "COLIMA", col = "darkgray", cex = .9, srt = 0 )
## dev.off()
...
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon == 14 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Jalisco", dn, "-", tmp))
plot(ed.map$jal, add = TRUE)
plot(se.map[se.map$disn==dn,], add = TRUE, border = "darkgray", col = portray[se.map$disn==dn]) # color nwin
#plot(ffcc, add = TRUE, lwd = .75)
#
plot(ed.map$nay, add = TRUE, lty = 1)
plot(ed.map$sin, add = TRUE, lty = 1)
plot(ed.map$dgo, add = TRUE, lty = 1)
plot(ed.map$ags, add = TRUE, lty = 1)
plot(ed.map$gua, add = TRUE, lty = 1)
plot(ed.map$col, add = TRUE, lty = 1)
