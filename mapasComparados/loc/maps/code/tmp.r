...
mu.map$mun <- c("ACAMBAY","ACULCO","ALMOLOYA DE ALQUISIRAS","AMANALCO","AMATEPEC","AMECAMECA","APAXCO","AXAPUSCO","COCOTITLAN","CUAUTITLAN","CHAPA DE MOTA","CHIAUTLA","DONATO GUERRA","ECATZINGO","ISIDRO FABELA","IXTAPAN DEL ORO","JALTENCO","JILOTEPEC","JILOTZINGO","JOQUICINGO","JUCHITEPEC","MELCHOR OCAMPO","MORELOS","NOPALTEPEC","OCUILAN","OTUMBA","OTZOLOAPAN","PAPALOTLA","POLOTITLAN","SAN SIMON","SANTO TOMAS","SOYANIQUILPAN","SULTEPEC","TEJUPILCO","TEMAMATLA","TENANCINGO","TEOLOYUCAN","TEPETLAOXTOC","TEPETLIXPA","TEXCALTITLAN","TEXCALYACAC","TIMILPAN","TLATLAYA","VALLE DE BRAVO","VILLA DE ALLENDE","VILLA DEL CARBON","VILLA VICTORIA","ZACAZONAPAN","ZACUALPAN","VALLE DE CHALCO SOLIDARIDAD","LUVIANOS","SAN JOSE DEL RINCON","COACALCO","TLALNEPANTLA","CHALCO","TEMASCALCINGO","EL ORO","ATLACOMULCO","JOCOTITLAN","IXTLAHUACA","JIQUIPILCO","ALMOLOYA DE JUAREZ","TEMASCALTEPEC","COATEPEC HARINAS","TONATICO","ZUMPAHUACAN","IXTAPAN DE LA SAL","VILLA GUERRERO","MALINALCO","ZINACANTEPEC","TOLUCA","TENANGO DEL VALLE","TIANGUISTENCO","XALATLACO","CALIMAYA","RAYON","ALMOLOYA DEL RIO","SAN ANTONIO LA ISLA","CAPULHUAC","OCOYOACAC","LERMA","NAUCALPAN","ATIZAPAN","CHAPULTEPEC","MEXICALTZINGO","METEPEC","SAN MATEO ATENCO","XONACATLAN","OTZOLOTEPEC","TEMOAYA","ATIZAPAN","HUIXQUILUCAN","NICOLAS ROMERO","TEPOTZOTLAN","CUAUTITLAN IZCALLI","TULTITLAN","TULTEPEC","ECATEPEC","ZUMPANGO","COYOTEPEC","HUEHUETOCA","TEQUIXQUIAC","HUEYPOXTLA","TEMASCALAPA","SAN MARTIN DE LAS PIRAMIDES","TEOTIHUACAN","TECAMAC","NEXTLALPAN","TONANITLA","ACOLMAN","TEZOYUCA","ATENCO","CHICONCUAC","TEXCOCO","IXTAPALUCA","NEZAHUALCOYOTL","CHIMALHUACAN","LA PAZ","CHICOLOAPAN","TENANGO DEL AIRE","TLALMANALCO","AYAPANGO","ATLAUTLA","OZUMBA","SAN FELIPE DEL PROGRESO")
...
sec2dis <- sec2dis[sec2dis$edon == 15,]
# send to seccion map
...
rm(tmp)
#di.map <- unionSpatialPolygons(se.map, se.map$disn, threshold=1) # proper way to get federal district objects... if only seccion shapefiles had no problems
#
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon== 15, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon==15,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/mex2015.csv", stringsAsFactors = FALSE) # 2015 districts
head(tmp)
tmp <- tmp[,c("seccion","disloc")] # drop munn
## tmp2 <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/mex2009.csv", stringsAsFactors = FALSE) # 2009 districts to cover missing secciones
## tmp2 <- tmp2[,c("seccion","disloc","cabloc")] # drop munn
## tmp <- rbind(tmp, tmp2)
## tmp <- tmp[duplicated(tmp$seccion)==FALSE,] # drop repeats
## tmp2 <- tmp[1:8,]; tmp2$seccion <- 6425:6432; tmp2$disloc <- tmp$disloc[tmp$seccion==6424]; tmp2$cabloc <- tmp$cabloc[tmp$seccion==6424];
## tmp <- rbind(tmp, tmp2) # fix missing (reseccionamiento) 
...
tmp <- data.frame(seccion=se.map$SECCION, edon=se.map$ENTIDAD, disn=se.map$disn, munn=se.map$MUNICIPIO, bastion=se.map$bastion2, ncasillas=sqrt(se.map$ncasillas), lon=tmp[,1], lat=tmp[,2], disloc=se.map$disloc, cabloc=se.map$cabloc, stringsAsFactors = FALSE)
...
#
# Estado
tmp <- as(ed.map$mex, "SpatialLinesDataFrame") # coerce polygons to lines 
...
# compress kml files to kmz DO THIS ON EMPTY MEMORY!
md2 <- c("/home/eric/Dropbox/data/elecs/MXelsCalendGovt/atlasDis/maps/")
library(plotKML)
setwd(paste(md2, "kml/", sep = ""))
kml_compress("mex.kml")
tgt <- c("ags","bc","col","df","gue","hgo","jal","mex","nay","nl","oax","ver")
for (i in 1:length(tgt)){
    #i <- 7 #debug or pick single state
    edo <- tgt[i]
    ## o <- "Bastiones"
    ## kml_compress(paste(edo, o, ".kml", sep = ""))
    kml_compress(paste(edo, ".kml", sep = ""))
}
...
text( x = -99.15, y = 19.3, labels = "D.F.", cex = .9, col = "darkgray", srt = 0 )
text( x = -99.1, y = 18.75, labels = "MORELOS", col = "darkgray", cex = .9 )
text( x = -98.55, y = 19, labels = "PUEBLA", col = "darkgray", cex = .9, srt = 80 )
text( x = -99, y = 20.2, labels = "HIDALGO", col = "darkgray", cex = .9, srt = -20 )
text( x = -98.57, y = 19.52, labels = "TLAX.", col = "darkgray", cex = .9, srt = -10 )
text( x = -100.5, y = 19.5, labels = "MICHOACAN", col = "darkgray", cex = .9, srt = 75 )
text( x = -100.55, y = 20.15, labels = "GUANAJUATO", col = "darkgray", cex = .9, srt = 45 )
text( x = -100.1, y = 20.3, labels = "QUERETARO", col = "darkgray", cex = .9, srt = 3 )
text( x = -99.75, y = 18.45, labels = "GUERRERO", col = "darkgray", cex = .9, srt = 25 )
## dev.off()

# plot same distrito only
# need to merge disn info into mun and sec object, in order to select just those belonging to dis
## pdf(file = paste(md2, "mex", dn, "-2.pdf", sep = ""))
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon==15 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("México", dn, "-", tmp))
...
loc <- c("topright","topleft","topright","bottomleft","topright",       #  5
         "topright","bottomright","topright","topright","bottomleft",   # 10
         "topright","topright","bottomleft","topleft","topright",       # 15
         "topright","topright","bottomright","topright","topright",     # 20
         "topleft","topright","bottomleft","topright","topright",       # 25
         "topright","topleft","topright","bottomright","bottomright",   # 30
         "topright","topright","bottomleft","topright","topright",      # 35
         "bottomright","topleft","bottomleft","topright","bottomright") # 40
## pdf(file = paste(md2, "mex", dn, "-3.pdf", sep = ""))
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon==15 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("México", dn, "-", tmp))
plot(ed.map$mex, add = TRUE)
