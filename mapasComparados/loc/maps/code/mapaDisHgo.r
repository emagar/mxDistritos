...
mu.map$mun <- c("ACATLAN","ACAXOCHITLAN","ACTOPAN","AGUA BLANCA","AJACUBA","ALFAJAYUCAN","ALMOLOYA","APAN","ATITALAQUIA","ATOTONILCO DE TULA","CALNALI","CARDONAL","CUAUTEPEC","CHAPANTONGO","CHAPULHUACAN","CHILCUAUTLA","EL ARENAL","ELOXOCHITLAN","EMILIANO ZAPATA","HUASCA","HUAUTLA","HUAZALINGO","HUEHUETLA","HUICHAPAN","IXMIQUILPAN","JACALA","JALTOCAN","JUAREZ HIDALGO","LA MISION","LOLOTLA","METEPEC","METZTITLAN","MINERAL DEL CHICO","MINERAL DEL MONTE","MIXQUIAHUALA","MOLANGO","NOPALA","OMITLAN","PACULA","PISAFLORES","PROGRESO","SAN AGUSTIN METZQUITITLAN","SAN BARTOLO TUTOTEPEC","SAN FELIPE ORIZATLAN","SAN SALVADOR","SANTIAGO DE ANAYA","TASQUILLO","TECOZAUTLA","TENANGO","TEPEAPULCO","TEPEHUACAN","TEPETITLAN","TETEPANGO","TEZONTEPEC","TIZAYUCA","TLAHUELILPAN","TLAHUILTEPA","TLANALAPA","TLANCHINOL","TLAXCOAPAN","TOLCAYUCA","TULANCINGO","VILLA DE TEZONTEPEC","XOCHIATIPAN","XOCHICOATLAN","ZAPOTLAN","ZEMPOALA","YAHUALICA","ZACUALTIPAN","ATOTONILCO EL GRANDE","TIANGUISTENGO","HUEJUTLA","FRANCISCO I. MADERO","MINERAL DE LA REFORMA","PACHUCA DE SOTO","TULA","SINGUILUCAN","EPAZOYUCAN","SANTIAGO TULANTEPEC","TEPEJI DEL RIO","ZIMAPAN","ATLAPEXCO","SAN AGUSTIN TLAXIACA","NICOLAS FLORES")
...
sec2dis <- sec2dis[sec2dis$edon == 13,]
# send to seccion map
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon== 13, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon==13,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/hgo2010.csv", stringsAsFactors = FALSE) # 2015 districts
...
tmp <- se.map@data; tmp <- tmp[, c("SECCION","MUNICIPIO", "disloc")]
table(tmp$MUNICIPIO, useNA = "always")
table(tmp$disloc, useNA = "always")
...
allSec <- allSec[allSec$edon==edon, c("edon","seccion","munn", "OBSERVACIONES", "action", "fr.to", "orig.dest", "when", "color", "coment")]
tmp <- tmp[, c("SECCION","disloc")]
allSec <- merge(x = allSec, y = tmp, all.x = TRUE, all.y = FALSE, by.x = "seccion", by.y = "SECCION")
colnames(allSec)
allSec <- allSec[, c("edon","seccion","munn", "disloc", "OBSERVACIONES", "action", "fr.to", "orig.dest", "when", "color", "coment")]
write.csv(allSec, file = "hgo2010.csv", row.names = FALSE)
#
...
#
# Estado
tmp <- as(ed.map$hgo, "SpatialLinesDataFrame") # coerce polygons to lines 
...
# grafica distritos de hidalgo 1 por 1
...
plot(ed.map$hgo, axes = TRUE, main = "Hidalgo (mapa 2006)")
...
plot(ed.map$gua, add = TRUE, lty = 3)
plot(ed.map$san, add = TRUE, lty = 3)
#text(coordinates(ed.map$tla), labels="Tlaxcala", cex=.75) # necesito modo de poner nombres de estados vecinos fácilmente
# 
plot(di.map, add = TRUE, border = "gray")
plot(di.map[di.map$DISTRITO==dn,], add = TRUE, border = "gray", col = "gray")
# thick state border
plot(ed.map$hgo, add = TRUE, lwd = 3)
plot(ed.map$hgo, add = TRUE, border = "red", lty = 3, lwd = 2)
...
text( x = -99.5, y = 19.65, labels = "MÉXICO", cex = .9, col = "darkgray", srt = -15 )
text( x = -98.2, y = 19.6, labels = "TLAXCALA", col = "darkgray", cex = .9 )
text( x = -99.7, y = 21, labels = "QUERÉTARO", col = "darkgray", cex = .9, srt = 65 )
text( x = -98.85, y = 21.4, labels = "SAN LUIS POTOSÍ", col = "darkgray", cex = .9, srt = 5 )
text( x = -98.1, y = 21.25, labels = "VERACRUZ", col = "darkgray", cex = .9, srt = -45 )
text( x = -98, y = 20, labels = "PUEBLA", col = "darkgray", cex = .9, srt = 75 )
## dev.off()
...
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon==13 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Hidalgo", dn, "-", tmp))
plot(ed.map$hgo, add = TRUE)
...
plot(ed.map$gua, add = TRUE, lty = 1)
...
loc <- c("topright","bottomright","topright","bottomright","bottomleft",
