...
sec2dis <- sec2dis[sec2dis$edon == 30,]
# send to seccion map
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon== 30, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon == 30,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
# FALTA SIGUIENTE RENGLON
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/ver.csv", stringsAsFactors = FALSE) # 2015 districts
...
#
# Estado
tmp <- as(ed.map$ver, "SpatialLinesDataFrame") # coerce polygons to lines 
...
plot(ed.map$ver, axes = TRUE, main = "Veracruz (mapa 2006)")
...
plot(ed.map$tab, add = TRUE, lty = 3)
# 
...
text( x = -95, y = 20.75, labels = "G O L F O   D E   M E X I C O", cex = .9, col = "deepskyblue", srt = -55 )
text( x = -98.525, y = 22.555, labels = "TAMAULIPAS", col = "darkgray", cex = .9 )
text( x = -98.9, y = 21.85, labels = "S.L.P.", col = "darkgray", cex = .9, srt = 80 )
text( x = -98.9, y = 20.5, labels = "HIDALGO", col = "darkgray", cex = .9, srt = 70 )
text( x = -98.2, y = 19.4, labels = "TLAX.", col = "darkgray", cex = .9, srt = -10 )
text( x = -98, y = 18.7, labels = "PUEBLA", col = "darkgray", cex = .9, srt = 0 )
text( x = -96.5, y = 17.5, labels = "OAXACA", col = "darkgray", cex = .9, srt = 0 )
text( x = -93.4, y = 17.3, labels = "CHIAPAS", col = "darkgray", cex = .9, srt = 55 )
text( x = -93.4, y = 18.15, labels = "TABASCO", col = "darkgray", cex = .9, srt = 10 )
## dev.off()
...
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon == 30 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Veracruz", dn, "-", tmp))
...
loc <- c("topright","bottomleft","topright","bottomleft","topright",
         "topright","bottomright","topright","topright","bottomright",
         "topright","topright","bottomleft","topright","topright",
         "topright","topright","bottomleft","topright","topright",
         "topright")
...
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Veracruz", dn, "-", tmp))
plot(ed.map$ver, add = TRUE)
plot(se.map[se.map$disn==dn,], add = TRUE, border = "darkgray", col = portray2[se.map$disn==dn]) # color 
#plot(ffcc, add = TRUE, lwd = .75)
#
plot(ed.map$tam, add = TRUE, lty = 1)
plot(ed.map$hgo, add = TRUE, lty = 1)
