...
sec2dis <- sec2dis[sec2dis$edon == 6,]
# send to seccion map
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon == 6, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon == 6,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/col2015.csv", stringsAsFactors = FALSE) # 2015 districts
head(tmp)
...
#
# Estado
tmp <- as(ed.map$col, "SpatialLinesDataFrame") # coerce polygons to lines 
...
# need to prune map to remove faraway isles
crop <- data.frame(x = c(-104.8,-103.4),  y = c(18.5,19.75))

## don't use loop for (dn in 1:8){
## # plot state map with highlighted district
## pdf(file = paste(md2, edo, dn, "-1.pdf", sep = ""))
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
plot(ed.map$col, axes = TRUE, main = "Colima (mapa 2006)",
          xlim = crop$x, ylim = crop$y)
plot(ed.map$jal, add = TRUE, lty = 3)
...
text( x = -104.4, y = 18.7, labels = "O C E A N O   P A C I F I C O", cex = .9, col = "deepskyblue", srt = -30 )
text( x = -104, y = 19.65, labels = "JALISCO", col = "darkgray", cex = .9 )
text( x = -103.5, y = 18.75, labels = "MICHOACAN", col = "darkgray", cex = .9, srt = 50 )
## dev.off()
...
# need to prune map to remove faraway isles
crop <- data.frame(x = c(-104.7,-103.6),  y = c(18.6,19.5))
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon==6 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Colima", dn, "-", tmp))#,
#               xlim = crop$x, ylim = crop$y)
