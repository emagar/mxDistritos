...
# read disloc2005
dl2015.map <- readOGR(dsn = tmp, layer = 'disloc2015')
colnames(dl2015.map@data) <- c("edon","edo","disloc")
# projects to a different datum with long and lat
dl2015.map <- spTransform(dl2015.map, osm()) # project to osm native Mercator


# add father/son info and dsi of mapLoc
dsi <- "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/ife.ine/mapasComparados/loc/simIndex/dist_col.csv"
...
sec2dis <- read.csv("/home/eric/Dropbox/data/elecs/MXelsCalendGovt/redistrict/ife.ine/mapasComparados/loc/colLoc.csv", stringsAsFactors = FALSE)
# send to seccion map
...
# shave map's bb to exclude isla revillagigedo
...
#plot(shave(p84(ed.map$col))) # debug
...
#plot(p84(ed.map$nay), col = "white", add = TRUE, lty = 3)
...
## m <- p84(shave(shave(dl.map[dl.map$disloc==dn,], p = .0125, s = 0),
##            p = .2, eastwest = FALSE)
##                ) # subsetted map for dn = 13 --- shave island
b <- as.data.frame(m@bbox)
...
bg <- bg.os
#
png(file = paste(md2, edo, dn, "-2.png", sep = ""), width=400, height=400, res=40) # see http://blog.revolutionanalytics.com/2009/01/10-tips-for-making-your-r-graphics-look-their-best.html
par(mar=c(0,0,2,0)) ## SETS B L U R MARGIN SIZES
tmp <-  dl.map$cab[which(dl.map$disloc==dn)]
tmp2 <- dl.map$dsi[which(dl.map$disloc==dn)]
plot(dl.map[dl.map$disloc==dn,], axes = TRUE,      main = paste("Colima ", dn, " - ", tmp, " (DSI = ", tmp2, ")", sep = ""))
## plot(shave(shave(dl.map[dl.map$disloc==dn,], p = .0125, s = 0),
##            p = .2, eastwest = FALSE), axes = TRUE, main = paste("Colima ", dn, " - ", tmp, " (DSI = ", tmp2, ")", sep = "")) # for dn = 13
plot(bg, add = TRUE)
#plot(dl.map[dl.map$disloc==dn,], lwd = 5, add = TRUE) # drop
plot(ed.map$col, add = TRUE)
...
#
sel <- which(dl2015.map$disloc==dl.map$father[dl.map$disloc==dn])
plot(dl2015.map[sel,], add = TRUE, lwd = 6, border = "red")
#
plot(dl.map[dl.map$disloc==dn,], add = TRUE, lwd = 4)
plot(mu.map, add = TRUE, border = "green", lwd = 1)
plot(mu.map, add = TRUE, lwd = 1, lty = 3)
plot(ed.map$col, add = TRUE, lwd = 3)
plot(ed.map$col, add = TRUE, border = "red", lty = 3, lwd = 2)
