...
sec2dis <- sec2dis[sec2dis$edon == 1,]
# send to seccion map
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = ncasillas[ncasillas$edon == 1, c("seccion","e12")], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
tmp <- data.frame(SECCION = se.map$SECCION)
tmp$orden <- 1:nrow(tmp)
tmp <- merge(x = tmp, y = nwin[nwin$edon == 1,], by.x = "SECCION", by.y = "seccion", all.x = TRUE, all.y = FALSE)
...
# add disloc
tmp <- read.csv(file = "/home/eric/Desktop/data/elecs/MXelsCalendGovt/redistrict/disloc/ags2013.csv", stringsAsFactors = FALSE)
...
#
# Estado
tmp <- as(ed.map$ags, "SpatialLinesDataFrame") # coerce polygons to lines 
...
plot(ed.map$ags, axes = TRUE, main = "Aguascalientes (mapa 2006)")
#plot(ed.map$jal, add = TRUE, lty = 3)
plot(ed.map$zac, add = TRUE, lty = 3)
# 
...
text( x = -102.7, y = 22.4, labels = "ZACATECAS", col = "darkgray", cex = .9, srt = 40 )
text( x = -102, y = 21.675, labels = "JALISCO", col = "darkgray", cex = .9, srt = 40 )
## dev.off()
...
par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
tmp <- cabDisNames$cab[which(cabDisNames$edon==1 & cabDisNames$disn==dn)]
plot(di.map[di.map$DISTRITO==dn,], axes = TRUE, main = paste("Aguascalientes", dn, "-", tmp))
...
## dev.off()
# legend(x="bottomleft", legend=rev(0:6), fill=rev(reds), bty="o", bg = "white", cex=.85)
