...
MagdalenaGironLopez es caso sencillo, cambia 2 distritos 17 18
AntonioPerezGomez es caso complejo, cambia 3 distritos 17 21 23


############################################################################
############################################################################
## close-up of munic and secc, colored by % indigenas, Magdalena Giron Lopez 
############################################################################
############################################################################
sel1 <- which(dle1.map$id==17 | dle1.map$id==18) # relevant 4 MagdaGiron
sel1s <- which(se.map$escenario1==17 | se.map$escenario1==18)
selm <- which(dlmgl.map$id==25 | dlmgl.map$id==26)
#plot(dle1.map[sel1,])
...
##     }
#
#pdf(file = paste(md2, edo, "mgl-p5licat.pdf", sep = ""))
#png(file = paste(md2, edo, "mgl-p5licat.png", sep = ""), width=10, height=10, units="cm", res=144) 
par(fig = c(0,1,0,1)) # sets primary plot size (to include smaller plot inside below)
par(mar = c(0,0,2,0)) ## SETS B L U R MARGIN SIZES
#par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
plot(p84(dle1.map[sel1,]), axes = TRUE, main = "Magdalena Girón López")#, bg = "lightblue")
#plot(bg, add = TRUE)
plot(p84(ed.map$oax), col = "white", add = TRUE, lty = 3)
plot(p84(ed.map$ver), col = "white", add = TRUE, lty = 3)
plot(p84(ed.map$tab), col = "white", add = TRUE, lty = 3)
...
plot(p84(dle1.map[sel,]), add = TRUE, border = "white", lwd = 4.5)
plot(p84(dlmgl.map[selm,]), add = TRUE, border = "white", lwd = 4.5)
plot(p84(dlmgl.map[selm,]), add = TRUE, border = "red", lwd = 3)
plot(p84(dle1.map[sel,]), add = TRUE, lwd = 1.5)
# municipal borders
...
# número efectivo d lenguas
#pdf(file = paste(md2, edo, "mgl-nel.pdf", sep = ""))
#png(file = paste(md2, edo, "mgl-nel.png", sep = ""), width=10, height=10, units="cm", res=144) 
par(fig = c(0,1,0,1)) # sets primary plot size (to include smaller plot inside below)
par(mar = c(0,0,2,0)) ## SETS B L U R MARGIN SIZES
#par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
plot(p84(dle1.map[sel1,]), axes = TRUE, main = "Magdalena Girón López")#, bg = "lightblue")
...
# lengua predominante
#pdf(file = paste(md2, edo, "mgl-lengMax.pdf", sep = ""))
#png(file = paste(md2, edo, "mgl-lengMax.png", sep = ""), width=10, height=10, units="cm", res=144) 
par(fig = c(0,1,0,1)) # sets primary plot size (to include smaller plot inside below)
par(mar = c(0,0,2,0)) ## SETS B L U R MARGIN SIZES
#par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
plot(p84(dle1.map[sel1,]), axes = TRUE, main = "Magdalena Girón López")#, bg = "lightblue")
...
# historia electoral
#pdf(file = paste(md2, edo, "mgl-core.pdf", sep = ""))
#png(file = paste(md2, edo, "mgl-core.png", sep = ""), width=10, height=10, units="cm", res=144) 
par(fig = c(0,1,0,1)) # sets primary plot size (to include smaller plot inside below)
par(mar = c(0,0,2,0)) ## SETS B L U R MARGIN SIZES
#par(mar=c(2,2,2,1)) ## SETS B L U R MARGIN SIZES
plot(p84(dle1.map[sel1,]), axes = TRUE, main = "Magdalena Girón López")#, bg = "lightblue")
