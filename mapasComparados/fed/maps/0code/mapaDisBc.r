...
mapOneDistrito <- function(dn = NULL, what2plot = NULL){
    ...
    if (dn==3){
        m <- p84(shave(df.map[df.map$disfed==dn,], p = .55, s = .1))  # subsetted map
    } else {
        shave(df.map[df.map$disfed==dn,], p = .55, s = .1)
    }
    ...
    if(dn==3){
        plot(shave(df.map[df.map$disfed==dn,], p = .55, s = .1), axes = TRUE, main = paste("Baja California ", dn, " - ", tmp, sep = "")) # for dn = 3
    } else {
        plot(df.map[df.map$disfed==dn,], axes = TRUE, main = paste("Baja California ", dn, " - ", tmp, sep = ""))
    }
    plot(bg, add = TRUE)
...
mapKeyPink(3)
mapOneDistrito(dn = 8, what2plot = 1)
