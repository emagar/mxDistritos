...
1.  [Description of *Recent Mexican electoral geography* repository](#org43dbc1e)
2.  [Files in the repository and how to cite them](#org9899f23)
    1.  [Measures of recent party performance for use in maps](#org7375ce2)
        1.  [Code](#orgd8de714)
        2.  [Data](#org3c46c34)
    2.  [Redistricting and *reseccionamiento*](#orgfcf6204)
    3.  [Comparative maps and shapefiles](#orgaf02d86)
    4.  [Descriptive plots and literature](#org18cf610)
3.  [Variables in the datasets ](#org45aeb4b)
    1.  [Observation identifiers](#orgaf011ac)
    2.  [Vote returns and party performance](#org0d2fabb)
    3.  [Redistricting and *reseccionamiento*](#orgf37f2a0)
4.  [Note on coalitions ](#org013b500)
5.  [Acknowledgements](#org5cb91ac)
...
-   `code/elec-data-for-maps.r` <a id="org1292815"></a> = code manipulates polling place vote returns in federal deputy elections 1994&#x2013;2018 for use in maps. Data are aggregated up to the municipal- and sección-levels for analysis.
...
Measures of federal deputy elections at two geographic levels are distributed: municipalities (files with `municipio` in name) and secciones electorales (files with `seccion` in name). Quantities of interest are available for 2009, 2012, 2015, and 2018. Variable descriptions [here](#org61f8234). 
...
# Variables in the datasets <a id="org61f8234"></a>
...
-   `pan` = vote share won by the PAN and allies, see the [note on parties and coalitions](#org0ed012e).
-   `pri` = vote share won by the PRI and allies, see the [note on parties and coalitions](#org0ed012e).
-   `morena` = vote share won by the left and allies, see [note on parties and coalitions](#org0ed012e).
...
-   `split` = equals 0 for secciones that remained unchanged in the period, otherwise indicates the year sección was split into smaller units due to oversize. Estimation of some of the quantities reported involved re-aggregating new units into their oversized parent sección in order to preserve the full vote returns time series. See [code](#org1292815) for details.
-   `new` = equals 0 for secciones that remained unchanged in the period, otherwise indicates the year sección was created by splitting an oversized sección into smaller units. Estimation of some of the quantities reported involved re-aggregating these smaller units into their oversized parent in order to preserve the full vote returns time series. See [code](#org1292815) for details.
...
# Note on coalitions <a id="org0ed012e"></a>
...
-   A residual "others" category sums the votes for parties other than those listed above for analysis. In 1991 the parties whose votes are summed are PARM, PDM, PFCRN, PPS, PEM, and PRT; in 1994 PPS, PFCRN, PARM, UNO-PDM, PT, and PVEM; in 1997 PC, PT, PVEM, PPS, and PDM; in 2000 PCD, PARM and DS; in 2003 PSN, PAS, MP, PLM, and FC; in 2006 PNA and ASDC; in 2009 PNA and PSD; in 2012 PNA only; in 2015 MC, PNA, PH, and a handful of independent candidates; and in 2018 a handful of independent candidates.  
    ...
    -   Variables in the dataset:<a id="orgd1cc36a"></a>
