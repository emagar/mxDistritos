...
############################
## sel2 <- which(!is.na(eq2$when3)) # two cases only
## eq2[sel2[1],]
# hand manipulation
...
sel <- which(eq2$action2=="split.to" & eq2$when2>2010 & eq2$when2 - eq2$when >= 10)
# monitor
eq2[sel,c(1,6:11)]
#
for (i in sel){
...
sel <- which(eq2$action2=="split.to" & eq2$when2==2010 & eq2$when2 - eq2$when >= 5)
# monitor
eq2[sel,c(1,6:11)]
#
...
eq2[sel,] <- manip # duplicate obs for manipulation --- sel can be reset
...
# indicate manipulation and return manipulation to data
manip <- within(manip, {
    action  <- "";    when  <- NA;   orig.dest  <- "";
    times.manip <- times.manip + 1;
})
